...
class MasterOrchestrator:
    ...
    def print_status(self):
        ...
        for name, info in status.items():
            ...
            print(f"{name:<20} {status_icon:<8} {info['status']:<10} {str(info['pid'] or 'N/A'):<8} {info['uptime']:<15} {info['restart_count']:<10} {health_icon}")
...
def main():
    ...
    if args.command == 'start':
        ...
        for name, healthy in results.items():
            icon = '[OK]' if healthy else ('[FAIL]' if healthy == False else '[-]')
            print(f"  {icon} {name}")
