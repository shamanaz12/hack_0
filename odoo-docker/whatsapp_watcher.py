...
class WhatsAppWatcher:
    ...
    def get_messages_from_chat(self) -> List[Dict]:
        ...
        FIX: Updated message selectors and proper error handling
        """
        messages = []
        
        try:
            # FIX: Updated message container selectors
            msg_selectors = [
            ...
            if not msg_containers:
                logger.debug("No message containers found")
                return []
            
            # Get last few messages
            for msg_container in msg_containers[-10:]:  # Last 10 messages
                try:
                    # FIX: Check for incoming messages (not sent by me)
